...
def transform_community_no_duplicate_real(input_file, output_file):
# tranform the community file to the designent file.
    fp_read = open(input_file)
    fp_write = open(output_file, 'w')
    node_list=set()
    ...
    for line in fp_read:
        ...
        if (node_id not in node_list):
            newline = node_id + ',' + str(int(cluster_id)-1) +'\n'
            fp_write.write(newline)                      
            node_list.add(node_id)
...
def transform_community_no_duplicate_no_use(input_file, output_file):
# tranform the community file to the designent file.
    fp_read = open(input_file)
    fp_write = open(output_file, 'w')
    node_list=set()
    ...
    print('clustering_ids:',cluster_ids)

    if ('0' not in cluster_ids):
        for line in fp_read:
...
def clusterByPopulation(output_folder, output_file, node_attibute, sample_clustering_file, inference_method, attribute_file):
    ...
    #netkit_directory="."
    mylist=['java','-Xmx2G', '-jar', 'NetKit-1.4.0.jar', '-attribute', node_attibute, '-known', sample_clustering_file, '-inferencemethod', inference_method, '-output', 'record', attribute_file]
    myorder=' '.join(mylist)
    print(myorder)
    process = subprocess.Popen(['java','-Xmx2G', '-jar', 'NetKit-1.4.0.jar', '-attribute', node_attibute, '-known', sample_clustering_file, '-inferencemethod', inference_method, '-output', 'record', attribute_file], cwd=netkit_directory, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
...
#    percentages = [0.15]
    for per in percentages:
        ...
        for rep in xrange(1, repeats+1):       
            ...
            for i in xrange(start_snapshot, snapshot): 
                ...
                t=time.time()                    
                ge=open(args.input_directory_stem + 'network_v'+ str(rep)+'.dat', 'rb')
    ...
    #                    snapshotCommunities(args.input_directory_stem + 'output-prefix.t0000'+ str(rep), partition_ground_truth)  
                        writePartitionToFile(partition_ground_truth, args.input_directory_stem + 'output-prefix.t0000'+ str(rep) +'.comms')   
                    if 1:
                        partition_ground_truth = community.best_partition(nx.Graph(original))
                        #expand partition
                ...
                #parseLancichinettiResults(args.input_directory_stem + prefix+'.t0'+ str(rep) + '.comm', args.input_directory_stem + 'output-prefix.t0000'+ str(rep) + '.comm')
                partition_set = se.ReadInData(args.input_directory_stem  + 'output-prefix.t0000'+ str(rep) +'.comms')
                partition_ground_truth = se.partitionFromFile(args.input_directory_stem + 'output-prefix.t0000'+ str(rep) +'.comms', ' ')
               ...
               #############original degree distribution
                ...
                #run blondel + TCI complete ,note that the index should start at 0 and consistent
                    ...
                    community_file= output_directory_stem + 'community_v'+ str(rep)+'.dat'
                    community_output= output_directory_stem + 'community_v'+ str(rep)+'.csv'
                    ########Realgraph clustering id from 1: 5000 ,need to minus 1
                    transform_community_no_duplicate_real(community_file, community_output)
                    ########benchmark graph clustering id from 0
                    # transform_community_no_duplicate(community_file, community_output)


                    #####transform edge file
                    edges_file= output_directory_stem + 'network_v'+ str(rep)+'.dat'
                    edges_output= output_directory_stem + 'network_v'+ str(rep)+'.rn'
                    ...
                    create_arff(output_directory_stem, 'network_v' + str(rep) + '.arff', 'community_v'+ str(rep)+'.csv', 'network_v'+ str(rep)+'.rn', nc, cluster_flag)
                    clusterByPopulation(output_directory_stem,'record.predict', 'Person:Behaviour', os.path.abspath(community_output2), 'relaxlabel', os.path.abspath(output_directory_stem +'network_v'+ str(rep)+'.arff'))
                ...
                #run blondel + lp  complete ,note that the index should start at 0 and consistent
                    ...
                    cluster_flag=0  #the clustering ID is ranked from 1 (big real_world), otherwises it is from 0 (syn)
                    create_arff(output_directory_stem, 'network_v' + str(rep) + '.arff', 'community_v'+ str(rep)+'.csv', 'network_v'+ str(rep)+'.rn', nc, cluster_flag)

                    clusterByPopulation(output_directory_stem,'record.predict', 'Person:Behaviour', os.path.abspath(community_output2), 'gibbs', os.path.abspath(output_directory_stem +'network_v'+ str(rep)+'.arff'))
                ...
                ####draw the degree distribution of sample.
                # plt.figure(2)
                ...
                ###################
                minimal_of_k=1
                orig_prefix ='orig_core'+str(rep)
                runKcore(args.input_directory_stem + 'network_v'+ str(rep) +'.dat', minimal_of_k, orig_prefix)
                ...
                sample_prefix = 'sample_core'+str(rep)
                ...
                #only draw the distribution of the first trial and first percentage and the last snapshot. Nice.        
                if per == percentages[0] and rep==1 and i== snapshot-1:  
        ...
        ####Repeat rep time for the average values stat############
        ###################################
        ###################################
        average_results=[]
        result_repeats=[]
        ...
        print('average trail results of a specfic method:', newstr)
        print('\n')
