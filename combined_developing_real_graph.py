...
#please note that when synthetic graph, we use binary_networks/. But when we use real graph, we use "/Networks_with_ground_truth_communities/"
...
#        print '\nTime to remove bench_directory: ' + str(bench_remove_end - bench_remove_start) + '\n'
    ...
    if 'generate_syn' in args.modules:
        # python generate.py -n 1000 --mu 0.40 -s 1 -e 10 -b binary_networks/ -o generated_benches/n_1000/
        # max_cluster_size_rate=[50, 70, 90, 110, 130, 150, 170, 190, 210]
        command_list = []
        command_list.append('python')
        command_list.append('generate_developing.py')
        command_list.append('-n')
        command_list.append(n)
        command_list.append('--maxk')
        command_list.append(str(int(0.10 * float(n))))
        ...
        command_list.append('--maxc')
        command_list.append(str(int(0.10 * float(n))))
        # command_list.append(str(max_cluster_size_rate[5]))  
        command_list.append('-s')
        command_list.append(str(args.start))
        command_list.append('-e')
        command_list.append(str(args.end))
        ...
        print '\nIncremented bench seed by: ' + str(increment_by) + '\n'
        touch('timeseed_inc_token_n_' + n + '_s_' + str(args.start) + '_e_' + str(args.end))
        order_generate = ' '.join(command_list) 
        subprocess.call(command_list)
        # bench_remove_start = time.clock()
        # shutil.rmtree(new_bench_directory_stem)
        # bench_remove_end = time.clock()
    ...
    ####If we have ground truth file we use the truth.
    for i in xrange(args.start, args.end + 1):
        if os.path.exists(working_directory+'community_v' + str(i) + '.dat'):
            ground_truth = ReadInData(working_directory+'community_v' + str(i) + '.dat')
            number_of_clusters = len(ground_truth)
            print 'the number of clusters is: '+str(number_of_clusters)
            # os.rename(working_directory+'community_v1.dat', working_directory +'output-prefix.t00000.comms')  
            shutil.copyfile(working_directory+'community_v' + str(i) + '.dat', working_directory +'output-prefix.t0000' + str(i) + '.comms')

        #####for synthetic we might transfer \tab to  ,but for real graph, let try it first.
        format_covert(working_directory)
        print('convert has been done')
    else:
        number_of_clusters = args.numcluster
        print 'the specfic number of clusters is: '+str(number_of_clusters)
...
#        command_list.append('0.7')
        ...
        command_list.append(str(number_of_clusters))
        bench_sample_start = time.time()
        subprocess.call(command_list)
        bench_sample_end = time.time()
        time_eclipse = format(bench_sample_end - bench_sample_start, '0.2f')

        print '\nTime to sample bench_directory: '+ args.sampling_condition + str(time_eclipse) + '\n'
        ...
        we=open('Time_sample_p_'+ str(int(100*float(args.samplerate))) + '_u_' + str(int(100*float(args.mu))) + '.csv','ab')
        we.write(args.sampling_condition)        
        we.write(','+str(time_eclipse))
        we.write('\n')
        we.close()
