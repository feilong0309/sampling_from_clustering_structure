...
#        percentages = [0.1, 0.3, 0.5, 0.7]
        sampling_conditions = ['induced_random_edge','induced_random_vertex','induced_weighted_random_vertex','kk_path','km_path','random_path','random_vertex','random_edge','random_walk','metropolis_subgraph','metropolized_random_walk','weighted_vertex','top_k_leader_balanced','top_k_leader_internal_priority','top_k_leader_internal_priority_extra','snowball_expansion','fast_subset_degree']
...
#                num_of_cluster=12
                ...
                delimeter = '\t'
                sample_size = int(math.ceil(float(numNodes)*float(val)))  
                sample_ = fast_subset_degree_sampler.fast_subset_degree_sampling(args.out_directory_stem + 'network_v' + str(i) + '.dat',sample_size,delimeter);  
            print time.time()-t  
